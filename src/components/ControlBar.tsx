...
export const ControlBar: React.FC<ControlBarProps> = ({
  ...
  const betSteps = [0.2, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 50.0, 100.0];

  const handleDecreaseBet = () => {
    if (isSpinning || isFreeSpinsActive || !onBetChange) return;
    const currentIdx = betSteps.findIndex((b) => Math.abs(b - currentBet) < 0.01);
    if (currentIdx > 0) {
      onBetChange(betSteps[currentIdx - 1]);
  ...
  const handleIncreaseBet = () => {
    if (isSpinning || isFreeSpinsActive || !onBetChange) return;
    const currentIdx = betSteps.findIndex((b) => Math.abs(b - currentBet) < 0.01);
    if (currentIdx >= 0 && currentIdx < betSteps.length - 1) {
      onBetChange(betSteps[currentIdx + 1]);
    } else {
